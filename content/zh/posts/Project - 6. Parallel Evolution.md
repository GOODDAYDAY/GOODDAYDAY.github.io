...
draft = false
title = '[Project] 6. 平行演化 — Skills 与 Harness 的 AI 协作实录'
...
- **改动太散**：LLM 一轮里又修 bug 又加功能又重构，没有分工，大改动容易翻车
- **Context 天花板**：项目一大就装不下，黄金时代很短暂
- **没有评判**：改得好不好全凭感觉，没有分数、没有标准
- **任务不分优先级**：不管什么任务都走同一套 plan → execute → evaluate，小改动绕不过 overhead
...
初版最简单的 while 循环能跑，但产出质量很随机。LLM 每次都自己判断"改什么"，一轮里又修 bug 又加功能又重构，改动范围不可控，经常一改就炸。三段式（bug_fix → features → polish）是第一次尝试给 LLM 的注意力加一个顺序约束：**先把坏的东西修好，再往上加新的，最后把整个盘子擦干净**。这个顺序不是拍脑袋想的，而是手工改代码时的自然优先级——你不会一边修 bug 一边加新功能。
...
- **固定顺序没有弹性**：不管你扔给它安全审计还是加一行日志，都走 bug_fix → features → polish，小任务被流程吃掉的时间比干活的时间还多
- **阶段间盲区**：每个阶段的 LLM 只看自己的 prompt 和当前代码，不知道前一个阶段做了什么决策、为什么——改了又改回去的循环反复出现
- **没有跳过机制**：如果当前代码没有任何 bug，`bug_fix` 阶段还是会跑——它找不到 bug 就随便找点东西改，反而可能引入新问题
- **分工本身模糊**：`quality_polish` 和 `bug_fix` 的边界是说不清的，cleanup 时把代码整理了一遍可能无意中修掉了一个 bug、也可能引入了一个新的

根源就一个：固定的三段式编排没有任何自适应能力。我意识到，与其给 LLM 规定"先做 bug_fix 再做 polish"，不如让它自己看代码、自己判断哪里需要改。于是有了 R3 的架构重构。
...
R3 同时砍掉了内置的辩论机制。原因很直接：**太慢了**。

每次 debate 要跑两个完整的 agent 调用——MVP agent 分析一遍，全功能 agent 分析一遍，然后评判 agent 再综合一遍。一个阶段跑下来，光辩论就要等好几个回合。小功能根本不需要这种开销——改个变量名也要拉三个 agent 辩论，纯粹浪费时间。而且 debater 的 prompt 很长，每轮 API 调用都在重发这些固定内容，token 成本叠加。

至于辩论质量——两个立场经常是稻草人互打。"这代码没问题"vs"这代码有问题"，脱离实际代码互相怼，产出的不是洞察而是文字量。

砍掉之后，流程从"先辩论再动手"变成"直接看代码、直接动手":
...
这三个"做不到"回头看，其实是**同一个问题的三个表象**：agent 没有能力判断自己改得好不好。它可以在代码上做任何事，但缺少一个足够好的反馈信号来告诉自己"这事做对了吗"。架构级创新需要胆量，胆量需要信心，信心来自准确的反馈。跨轮次记忆如果存的是"改完发现没什么用"的经验，存了也没价值——**记忆的有效性取决于评判的准确性**。评判信号是整个自改进体系的咽喉——这条咽喉不打通，上面所有能力都被卡住。

#### 迭代慢的根因分析

跑到 cycle 30-40 左右，迭代明显变慢了。表面看是"代码趋于稳定"，但真正的分水岭是：**低垂的果实摘完了**。

前几十轮 agent 做的事——修语法错误、删死代码、合并重复逻辑、补缺失的测试——这些都是"看到就能修"的问题，不需要品味。摘完之后，剩下的全是需要判断力的改进：这个模块边界画对了吗？这个抽象层次合适吗？这个接口设计够不够泛化，还是过度设计了？这些问题**没有标准答案**，需要 taste。

而 dual evaluator 的设计初衷是抓缺陷（安全漏洞、逻辑错误、波及效应），它根本设计来回答"这个架构决策好不好"。所以 agent 越往后越慢——不是它累了，是**评判系统提供不了它需要的信号**。它面对的问题变难了，但评判工具没跟着变强。

**这一版的缺陷**：

- **评判信号越来越弱**：dual evaluator 的分数集中在 5-7 分，区分不了"还行"和"真好"，agent 没有明确的优化方向
- **没有跨轮次记忆**：每轮 agent 都是失忆状态，上一轮犯的错下一轮继续犯，经验无法积累
- **只会做减法**：没人给方向时，agent 只会打磨已有代码——删死代码、补测试、合并重复——永远不会主动提出"要不要试试一种新的模块划分方式"
- **评判标准不可变**：evaluator prompt 和权重硬编码在 Python 文件里，agent 什么代码都能改，唯独改不了评判自己的标准
...
- **没有跨 session 记忆**：每次 pipeline 跑完，agent 学到的东西就丢了，下一次从零开始——指标能告诉你"这一轮做对了什么"，但不能告诉你"上一轮做对的东西下一轮能不能复用"
- **评估器校准还靠手动**：calibration benchmark 框架搭好了，但没跑出足够的数据来真正校准评分分布，评判质量没有闭环
- **依然做不出加法**：指标让减法（清理、优化）更精准了，但"加一个新能力、设计一个新模块"还是得人来推——方向还是人给的
- **评判标准和 agent 是分离的**：evaluator 怎么评，agent 管不了；agent 想调整优化方向，只能等人在 pipeline JSON 里改配置

方向给对了，指标驱动的迭代确实比 Phase 3 纯自由探索高效。但根本问题还在：**谁来定义方向、谁来校准评判、什么时候能把这个权力交一部分给 agent 自己**。这就是 V5 要试的。
