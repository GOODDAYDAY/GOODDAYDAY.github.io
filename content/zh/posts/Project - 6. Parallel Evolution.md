...
今年之前，一直也在用 AI 来帮忙做各种各样的东西，说实话属于不掉队、啥都了解一点的状态。正是 OpenClaw 的大火，才发现自己一直想要做的事情，它来了——或者说，别人把雏形搞出来了。

想做的事很简单：**把项目信息、个人偏好、公共约定，一次性写进一个基础文件，然后在每次使用 AI 时自动注入到上下文里**。这件事之前一直没做成——门槛太高，在公司里推不动。等 Claude Code 的 skills/commands 概念出来，发现这不就是吗？

### 2.2 Claude Code 的体验升级

Claude Code 带来的体验升级没得挑：
- 自我编排能力强，能做整体分析
- Context 管理更智能
- sub-agent 的支持让并行任务真正可用

用过的都说好，这也是后续所有工作方式的起点。

## 3. 项目一：Skills 仓库的演化

### 3.1 阶段一——手打 Prompt 时代
...
多数情况下的融合方案确实不错。但这个模式后来也暴露了问题：**它慢，而且大多数小功能根本不需要辩论**。
...
### 4.4 阶段三——自我编排（R3 至 cycle 67，后进入阶段四）
...
针对 4.4.4 里的两个核心问题，最近在代码里落地了第一批指标驱动的组件：
