...
R3 同时砍掉了内置的辩论机制。为什么要砍？

初版没有足够强的代码检查工具，LLM 看不到全局，debate 的作用是**用对话摩擦逼出更周全的判断**——两个视角互怼，暴露彼此忽略的问题。有用，但代价很大：每次 debate 要跑两轮完整的 agent 调用，而且两个立场（"这代码没问题"vs"这代码有问题"）经常是稻草人互打，脱离实际代码。

到了 R3，agent 的工具链已经成熟——`grep_search` 能精确找调用点、`cross_reference` 能画依赖图、`symbol_extractor` 能提取符号。agent 不需要凭空辩论了，**它能直接看代码**。看代码比辩论高效得多：辩论产出的是观点，代码检查产出的是事实。

砍掉 debate 之后，流程从"先辩论再动手"变成"先看代码再动手":
