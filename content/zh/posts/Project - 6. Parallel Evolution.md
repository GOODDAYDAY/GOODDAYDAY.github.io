...
举个例子，如果要写一个用户注册接口，当时的对话大概是：
```
我：项目是 Java Spring + MySQL，帮我在 UserController 里加一个注册接口。
    PS：按三层结构（Controller/Service/Repository）来写。
AI：好的，这是 Controller 层...
我：等等，先在 requirements/REQ-xxx/ 下建需求文档。
AI：好的...
我：还有，不要用 LomboK。
```
每次都要重新交代一遍项目背景和约定。更重要的是——**AI 没有上下文连贯性**，下次对话又要从头说起。
...
拿"用户注册"这个需求举例，辩论过程大概是：
```
MVP Agent："只做用户名+密码注册，邮箱验证后续再补。早点上线验证需求。"
全功能 Agent："一次性实现邮箱+手机号+OAuth 三方登录，
    等用户量上来再改认证系统代价太大了。"
评判 Agent："第一期先做邮箱注册，后端预留 OAuth 扩展点。
    这样既快速上线，又不会以后改不动。"
```
多数情况下融合方案确实比任何一方的单一提议都好。但这个模式后来也暴露了问题：**它慢，而且大多数小功能根本不需要辩论**。
...
同样说"用户注册"，编排者的反应完全不同了：
```
我："加一个用户注册功能。"
编排者：→ 分类 B（新功能）→ 创建 REQ-008 → 走完整流水线

我："把注册改成支持手机号。"
编排者：→ 分类 C（规格变更）→ 定位到 REQ-008 → 跳过 analyze → 从 tech 开始

我："注册页面的按钮换个颜色。"
编排者：→ 分类 A（Trivial）→ 直接改，不建 REQ
```
...
比如一个简单的"接口加个日志"，在早期版本里会这样走：
```
1. req-analyze 启动 → 双 Agent 辩论"加日志的需求分析"
   （MVP："不够，还要加监控。" 全功能："不够，还要加链路追踪。"）
2. req-tech 启动 → 又一轮辩论"日志方案选哪个"
3. 然后才进 req-code
```
前后折腾 5 分钟，而写 `log.info()` 本身只要 30 秒。**辩论的 overhead 在小需求上完全不成比例。**
...
```
LLM 分析代码后决定：
→ read_file('harness/tools/base.py')    # 看当前工具实现
→ edit_file(...)                        # 加一个新工具方法
→ bash('pytest tests/')                 # 跑测试验证
→ read_file('harness/config.py')        # 看完配置再决定下一步
→ ... 循环，直到达到 max_tool_turns
```
每回合 LLM 自己决定"下一步看什么、改什么"，最核心的 `llm.py` 就是驱动这个循环的 60 行代码。
...
不再有人告诉 agent "这轮你要修 bug"还是"这轮你要加功能"。看看实际的一次 cycle 日志：
```
Cycle 42 — agent 读完代码后决定：
"tools/base.py 的路径安全检查有重复逻辑。
重构：提取 _check_path 公共方法，统一错误处理。"
→ 改了 3 个文件
→ 新增 12 个测试
→ 评估分：7.8/10
→ 耗时：4 分 32 秒
...
现在的 cycle_metrics 会记录类似这样的数据：
```
上下文质量：    8.2/10  （代码读取充分，理解准确）
记忆与学习：    7.5/10  （记住了之前的架构决策，没有重复犯错）
评估一致性：    6.8/10  （还需校准，偶有误判）
改动影响范围：  3 个文件
测试覆盖率：    +4.2%
```
有了这些，evaluator 有了具体的评判目标，agent 也有了"做对了什么"的反馈，而不是在真空里自由发挥。
...
<img src="/images/mermaid/evolution-zh-16.svg" alt="Harness 四个版本演化时间线" style="max-width:100%;">

**Figure 4.8 — Harness 四个版本演化时间线**

Harness 走完了四个阶段，Skills 也走完了五个阶段。看上去是两个独立的技术故事，没什么特别的。但把两个项目的时间线叠在一起，我发现了让自己都没想到的事。
...
| **当前状态** | 稳定使用，持续迭代 | 指标驱动阶段：intel_metrics / cycle_metrics / benchmarks 已落地 |
...
强烈推荐每个长期使用 Claude Code 的程序员维护自己的 skills 仓库：
- **不用每次手打**：只要 skill 写好了，后续都是调用
- **跟着你一起成长**：技术认知提升，skills 跟着升级
- **是你对 AI 理解力的侧面体现**：一个人的 skill 质量，几乎直接反映了他对 AI 协作的理解深度
- **能共享、能团队化**：写好的 skill 可以分享，变成团队的公共财富
